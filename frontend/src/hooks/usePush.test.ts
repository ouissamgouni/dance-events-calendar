...
afterEach(() => {
  ...
  if ('serviceWorker' in navigator) {
    // @ts-expect-error — test-only cleanup of stubbed navigator API.
...
describe('usePush', () => {
  ...
  it('enable() requests permission, subscribes, and POSTs the subscription', async () => {
    ...
    let posted: { endpoint?: string; keys?: Record<string, string> } | null = null
    server.use(
      http.get('*/api/push/vapid-public-key', () =>
        HttpResponse.json({ public_key: VAPID_KEY }),
      ),
    ...
    await waitFor(() => expect(result.current.status).toBe('on'))
    expect(posted?.endpoint).toBe('https://push.example/abc')
    expect(posted?.keys).toEqual({ p256dh: 'p256', auth: 'authsecret' })
  })

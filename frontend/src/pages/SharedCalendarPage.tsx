            ...
            <main className="mx-auto max-w-7xl px-4 py-4">
                ...
                <div className="mb-4 flex flex-wrap items-center gap-3">
                    ...
                    <h1 className="text-lg font-bold text-ink flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-ink">
                ...
                {/* Shared-link banner */}
                {!loading && !notFound && (
                    <div className="mb-4 rounded-lg border border-line bg-surface px-4 py-2 text-sm text-ink-soft">
                        This is a shared calendar. Save individual events to add them to{' '}
                        <Link to="/my-events" className="text-action hover:underline">
                            your own calendar
                        </Link>
                        .
                    </div>
                ...
                {loading && (
                    <p className="text-center text-muted py-12">Loading shared calendar…</p>
                )}

                {!loading && notFound && (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        <p className="text-ink-soft text-lg font-medium">Share link not found</p>
                        <p className="text-muted text-sm mt-1">
                            This link may be invalid or the calendar may have been removed.
                        </p>
                        <Link to="/" className="mt-6 text-sm text-ink-soft hover:underline">
                            ← Browse events
                        </Link>
                    </div>
                )}
                ...
                {!loading && !notFound && events.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <p className="text-ink-soft text-lg font-medium">No events saved yet</p>
                        <p className="text-muted text-sm mt-1">

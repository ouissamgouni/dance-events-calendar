...
describe('SquareAreaMapEditor', () => {
    ...
    it('renames meaningful movement and prioritizes corner resizing', async () => {
        const onChange = vi.fn();
        render(
            <SquareAreaMapEditor
                area={bboxSearchArea({ label: 'Europe', min_lat: 20, min_lng: -10, max_lat: 65, max_lng: 40 }, 'preset')}
                onChange={onChange}
            />,
        );
        ...
        const handle = screen.getByRole('button', { name: 'Resize area from north west' });

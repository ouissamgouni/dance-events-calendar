    ...
    return (
        <div className="fixed inset-0 z-40 flex">
            ...
            <div className="w-full max-w-2xl bg-surface shadow-xl flex flex-col">
                ...
                {activeTab !== 'history' && (
                    <div className="px-4 py-2 border-b border-card-line">
                        <input
                            type="text"
                            value={q}
                ...
                <div className="flex-1 overflow-y-auto">
                    {loading ? (
                                ...
                                {history.map((h) => (
                                    <li key={h.id} className="p-3 text-xs">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <span className="font-medium text-ink capitalize">{h.scan_type}</span>
                        ...
                        <div className="p-6 text-center text-xs text-muted">No series groups</div>
                    ) : (
                        <ul className="divide-y divide-slate-100">
                            {groups.map((g) => (
                                activeTab === 'resolved' ? (
                                    <li key={g.id} className="px-3 py-2">
                                        ...
                                        >
                                            {g.canonical_title}{' '}
                                            <span className="text-muted">({g.events.length})</span>
                                        </button>
                                    </li>
                                ) : (

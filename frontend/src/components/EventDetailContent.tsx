...
export default function EventDetailContent({
    ...
    // Fetch message count to conditionally show the Messages section
    React.useEffect(() => {
        let cancelled = false;
        fetchEventMessages(event.event_id, { limit: 1, offset: 0 })
            .then((res) => {
                if (cancelled) return;
                setMessageCount(res.total);
            })
            .catch(() => {
                if (cancelled) return;
                setMessageCount(0);
    ...
    return (
        <div className={showActions ? 'space-y-3' : 'space-y-4'}>
            ...
            {showActions && showRatings && (() => {
                // Determine if reviews exist based on auth state
                const hasReviews = user
                    ? aggregate && aggregate.count > 0 && aggregate.display_state !== 'none'

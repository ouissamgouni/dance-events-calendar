...
export function useAnchoredToast(anchorRef: RefObject<HTMLElement | null>) {
    ...
    const node = visible && pos ? createPortal(
        <div
            ...
            className={`pointer-events-none z-[12000] text-center text-[11px] font-medium leading-snug text-ink bg-surface/75 backdrop-blur-sm px-2.5 py-1 shadow-md ring-1 ring-slate-200/70 transition-opacity duration-200 ${fading ? 'opacity-0' : 'opacity-100'}`}
        >
            {message}
        </div>,
        document.body,

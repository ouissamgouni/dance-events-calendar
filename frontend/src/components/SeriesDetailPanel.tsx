            ...
            <div className="w-full max-w-xl bg-white shadow-xl flex flex-col">
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-semibold text-slate-800">Series details</h2>
                    </div>
                    <button onClick={onClose} className="text-slate-400 hover:text-slate-700 text-sm px-2">✕</button>
                </div>

                {error && (
                    <div className="px-4 py-2 text-xs text-red-600 border-b border-red-200 bg-red-50">{error}</div>
                )}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    {/* Add member */}
                    <div>
                        <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide mb-2">Add event</p>
                        <input
                            type="text"
                            value={memberSearch}
                        ...
                        {memberSearch.trim().length >= 3 && (
                            searching && candidates.length === 0 ? (
                                <p className="mt-2 text-[11px] text-slate-400">Searching…</p>
                            ) : candidates.length > 0 ? (
                                <div className="mt-2 max-h-48 overflow-y-auto border border-slate-200">
                                    {candidates.map((ev) => (
                                        <button
                                            key={ev.event_id}
                                            onClick={() => addMember(ev.event_id)}
                                            disabled={acting}
                                            className="flex w-full items-center justify-between gap-2 border-b border-slate-100 px-2 py-1.5 text-left last:border-b-0 hover:bg-blue-50 disabled:opacity-50"
                                        >
                                            <span className="min-w-0 flex-1 truncate text-[11px] text-slate-700">{ev.title}</span>
                                            <span className="text-[10px] text-slate-400">{new Date(ev.start).toLocaleDateString()}</span>
                                        </button>
                                    ))}
                                </div>
                            ) : (
                                <p className="mt-2 text-[11px] text-slate-400">No events found.</p>
                            )
                        )}
                    </div>

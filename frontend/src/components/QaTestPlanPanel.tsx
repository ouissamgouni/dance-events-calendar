...
function QaTestPlanPanelInner({
    ...
    const panelContent = (
        <>
            {/* Header */}
            <div className="flex items-center justify-between px-3 py-2 border-b border-line bg-canvas sticky top-0 z-10">
                ...
                <div className="flex items-center gap-0.5">
                    <button
                        onClick={togglePin}
                        className={`p-1 rounded transition-colors ${pinned ? 'text-indigo-600 bg-indigo-50 hover:bg-indigo-100' : 'text-muted hover:bg-canvas hover:text-ink-soft'}`}
                    ...
                    <button
                        onClick={handleClose}
                        className="p-1 rounded hover:bg-canvas transition-colors text-ink-soft"
                    >
            ...
            <div className="p-3 overflow-y-auto flex-1">
                ...
                {(!activeScenario || (hasMultiple && !testPlan)) && (
                    <div className="mb-4">
                        <p className="text-xs text-ink-soft mb-2">Select a scenario:</p>
                            ...
                            {scenarios.map((s) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className="p-3 border border-line rounded-md cursor-pointer hover:bg-indigo-50 hover:border-indigo-300 transition-colors"
                                    >
                                        <div className="flex justify-between items-center">
                                            <span className="text-sm font-bold">{s}</span>
                                            {sDone > 0 && (
                                                <span className="text-xs bg-green-100 text-success px-1.5 py-0.5 rounded">
                                                    {sDone} done
                                                </span>
                                            )}
                                        </div>
                ...
                {hasMultiple && activeScenario && testPlan && (
                    <button
                        ...
                        className="text-xs text-ink-soft hover:text-ink mb-3 flex items-center gap-1"
                    >
                ...
                {loading && (
                    <div className="flex justify-center py-8">
                        <div className="w-6 h-6 border-2 border-line border-t-indigo-500 rounded-full animate-spin" />
                    </div>
                )}
                ...
                {testPlan && (
                    <div>
                        ...
                        <p className="text-xs text-ink-soft mb-4">{testPlan.description}</p>
                        <span
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start gap-2">
                                        <div className={`mt-0.5 flex-shrink-0 ${isDone ? 'text-green-500' : 'text-muted'}`}>
                                            {isDone ? (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-ink-soft mb-1">{step.description}</p>
                                            <div className="bg-blue-50 px-2 py-1 rounded-sm mb-1">
                                                {typeof step.expected === 'string' ? (
                                                    ...
                                                    <div className="text-xs text-action space-y-1">
                                            ...
                                            <p className="text-xs text-ink-soft italic">✓ {step.verification}</p>
                                        </div>
                                    </div>
                                </div>
                            );
    ...
    if (pinned) {
        return (
            <div
                className="fixed right-0 top-0 bottom-[22px] bg-surface z-10 flex flex-col border-l border-line"
    ...
    return (
        <>
            ...
            {/* Panel */}
            <div
                className="fixed right-0 top-0 bottom-[22px] bg-surface shadow-xl z-[200] flex flex-col border-l border-line"
